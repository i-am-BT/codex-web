...
app.post('/api/chat', requireAuth, (req, res) => {
  ...
  const startContent = `任务开始\nprovider=${provider || 'default'} model=${model} reasoning=${reasoningEffort || 'default'} sandbox=${sandbox} cwd=${cwd}`;
...
function writeCodexConfig(providers, defaultProvider, defaultModel) {
  ...
  backupCodexConfig();
  const reasoningEffort = readCodexDefaults().reasoningEffort;
...
function cleanReasoningEffort(value) {
  ...
  if (!effort) return '';
  if (!['low', 'medium', 'high', 'xhigh'].includes(effort)) throw new Error('思考档位无效');
  return effort;
}
...
async function boot(selectRecent=false){const res=await fetch('/api/config');if(!res.ok)return;const data=await res.json();appearance=data.appearance||appearance;applyAppearance();cwd.value=data.defaults.cwd;sandbox.value=data.defaults.sandbox;approval.value=data.defaults.approval;reasoningEffort.value=data.defaults.reasoningEffort||'';provider.innerHTML='<option value="">默认</option>';for(const p of data.providers){const opt=document.createElement('option');opt.value=p;opt.textContent=p;provider.appendChild(opt)}provider.value=data.defaults.provider||'';await loadModels(provider.value,data.defaults.model);renderHistory(data.conversations);updateSafetyHint();if(selectRecent&&data.conversations.length)await loadConversation(data.conversations[0].id)}
...
function stopRunningStatusPoll(){if(runningStatusTimer){clearTimeout(runningStatusTimer);runningStatusTimer=null}}
function pollRunningConversation(id,seq){stopRunningStatusPoll();runningStatusTimer=setTimeout(async()=>{runningStatusTimer=null;if(seq!==conversationLoadSeq||currentConversationId!==id)return;try{const res=await fetch('/api/conversations/'+encodeURIComponent(id));if(!res.ok)throw new Error('状态检查失败');const data=await res.json();if(seq!==conversationLoadSeq||currentConversationId!==id)return;if(data.conversation.status==='running'){pollRunningConversation(id,seq);return}await loadConversation(id);await refreshHistory();playTaskCompleteSound()}catch(e){if(seq===conversationLoadSeq&&currentConversationId===id)pollRunningConversation(id,seq)}},1500)}
function newChat(){stopRunningStatusPoll();conversationLoadSeq++;currentConversationId='';updateActiveHistory();setRunningState(false);chat.innerHTML='<div class="empty"><b>Ask Codex</b><span>选择目录和模型，然后发送任务。</span></div>';statusEl.textContent='New chat';input.value='';input.style.height='auto';clearPendingAttachments();closeMenu();input.focus()}
function scrollChatToLatest(){requestAnimationFrame(()=>{chat.scrollTop=chat.scrollHeight})}
function setRunningState(running){sendBtn.disabled=running;cancelBtn.disabled=false;cancelBtn.classList.toggle('hidden',!running);if(running)statusEl.textContent='Running'}
async function loadConversation(id){stopRunningStatusPoll();const seq=++conversationLoadSeq;currentConversationId=id;updateActiveHistory();statusEl.textContent='Loading...';const res=await fetch('/api/conversations/'+encodeURIComponent(id));if(seq!==conversationLoadSeq)return;if(!res.ok){statusEl.textContent='加载失败';return}const data=await res.json();if(seq!==conversationLoadSeq)return;currentConversationId=data.conversation.id;updateActiveHistory();chat.innerHTML='';(data.conversation.messages||[]).forEach((msg,index)=>addMsg(msg.role==='log'?'log':msg.role,msg.content,{messageIndex:index}));const running=data.conversation.status==='running';setRunningState(running);if(running){const indicator=addMsg('assistant','');indicator.innerHTML='<span class="spinner"></span> Codex 正在运行...';pollRunningConversation(data.conversation.id,seq)}else{statusEl.textContent='Loaded '+new Date(data.conversation.updatedAt||data.conversation.createdAt).toLocaleString()}closeMenu();scrollChatToLatest()}
