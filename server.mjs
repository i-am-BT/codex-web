...
app.post('/api/models', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const models = [...new Set([...savedModels, ...remoteModels].filter(Boolean))];
    res.json({ ok: true, models });
  } catch (err) {
    if (providerName) {
      const savedModels = getProviderModels(providerName);
      if (savedModels.length) return res.json({ ok: true, models: savedModels, warning: err.message });
    }
    res.status(502).json({ error: err.message });
  }
});
...
app.post('/api/providers', requireAuth, requireConfigWrite, (req, res) => {
  ...
  if (!apiKey) return res.status(400).json({ error: 'API Key 不能为空' });
  if (!models.length) return res.status(400).json({ error: '请至少填写一个模型' });
...
function normalizeProviderModels(models, fallbackModel = '') {
  ...
  const cleaned = [];
  for (const item of [...values, fallbackModel]) {
    const model = cleanValue(item);
    if (model && !cleaned.includes(model)) cleaned.push(model);
  }
  return cleaned.slice(0, 50);
}

function readProviderModelsStore() {
  try {
...
function writeProviderModelsStore(store) {
  writeFileSync(PROVIDER_MODELS_FILE, JSON.stringify(store, null, 2), { mode: 0o600 });
}

function getProviderModels(name) {
  const provider = cleanProviderName(name);
  if (!provider) return [];
  return readProviderModelsStore()[provider] || [];
}

function saveProviderModels(name, models) {
  const provider = cleanProviderName(name);
  if (!provider) return [];
  const list = normalizeProviderModels(models);
  const store = readProviderModelsStore();
  if (list.length) store[provider] = list;
...
function deleteProviderModels(name) {
  const provider = cleanProviderName(name);
  if (!provider) return;
  const store = readProviderModelsStore();
...
const provider = document.getElementById('provider'), model = document.getElementById('model'), reasoningEffort = document.getElementById('reasoningEffort'), cwd = document.getElementById('cwd'), sandbox = document.getElementById('sandbox'), approval = document.getElementById('approval'), history = document.getElementById('history'), providerForm = document.getElementById('providerForm'), providerMsg = document.getElementById('providerMsg'), newProviderModelRows = document.getElementById('newProviderModelRows'), defaultMsg = document.getElementById('defaultMsg'), safetyHint = document.getElementById('safetyHint');
...
providerForm?.addEventListener('submit', async(e)=>{e.preventDefault();providerMsg.textContent='保存中...';const models=collectNewProviderModels();if(!models.length){providerMsg.textContent='请至少填写一个模型';return}const payload={name:document.getElementById('newProviderName').value,baseUrl:document.getElementById('newProviderUrl').value,apiKey:document.getElementById('newProviderKey').value,model:models[0],models,wireApi:document.getElementById('newProviderWire').value};const res=await fetch('/api/providers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const data=await res.json();if(!res.ok){providerMsg.textContent=data.error||'保存失败';return}providerMsg.textContent='已保存';document.getElementById('newProviderKey').value='';resetNewProviderModelRows();await boot();provider.value=data.provider;await loadModels(data.provider,data.model);});
document.getElementById('addProviderModel')?.addEventListener('click',()=>{addNewProviderModelRow('');const rows=newProviderModelRows?.querySelectorAll('.newProviderModelInput');rows?.[rows.length-1]?.focus()});
document.getElementById('fetchNewModels')?.addEventListener('click', async()=>{providerMsg.textContent='获取模型中...';const data=await requestModels({baseUrl:document.getElementById('newProviderUrl').value,apiKey:document.getElementById('newProviderKey').value});if(data.error){providerMsg.textContent=data.error;return}const list=document.getElementById('newProviderModelList');if(list){list.innerHTML='';for(const item of [...new Set((data.models||[]).filter(Boolean))]){const opt=document.createElement('option');opt.value=item;list.appendChild(opt)}}const current=collectNewProviderModels();if(!current.length&&data.models?.[0])setNewProviderModels([data.models[0]]);providerMsg.textContent=data.models.length?'已获取 '+data.models.length+' 个模型，可下拉选择或点 + 手动添加':'没有返回模型，可点 + 手动添加模型名';});
...
function collectNewProviderModels(){return [...new Set([...(newProviderModelRows?.querySelectorAll('.newProviderModelInput')||[])].map((input)=>String(input.value||'').trim()).filter(Boolean))]}
function addNewProviderModelRow(value=''){if(!newProviderModelRows)return;const row=document.createElement('div');row.className='providerModelRow';const input=document.createElement('input');input.className='newProviderModelInput';input.setAttribute('list','newProviderModelList');input.placeholder='例如 gpt-5.4，可手动填写或先获取';input.autocomplete='off';input.value=value||'';const remove=document.createElement('button');remove.type='button';remove.className='miniDanger providerModelRemove';remove.title='删除模型';remove.setAttribute('aria-label','删除模型');remove.textContent='×';remove.addEventListener('click',()=>{const rows=newProviderModelRows.querySelectorAll('.providerModelRow');if(rows.length<=1){input.value='';input.focus();return}row.remove()});row.append(input,remove);newProviderModelRows.appendChild(row)}
function setNewProviderModels(models){if(!newProviderModelRows)return;const list=[...new Set((models||[]).map((item)=>String(item||'').trim()).filter(Boolean))];newProviderModelRows.innerHTML='';if(!list.length){addNewProviderModelRow('');return}for(const item of list)addNewProviderModelRow(item)}
function resetNewProviderModelRows(){setNewProviderModels(['']);const list=document.getElementById('newProviderModelList');if(list)list.innerHTML=''}
