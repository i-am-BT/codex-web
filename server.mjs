...
.menuBtn{display:none}.scrim{display:none}@media(max-width:820px){.app{display:block}.menuBtn{display:grid;place-items:center;width:42px;height:42px;border-radius:13px;background:#101722;border:1px solid var(--line);color:var(--text);font-size:24px;line-height:1}.side{display:flex;position:fixed;z-index:30;left:0;top:0;bottom:0;width:min(86vw,330px);transform:translateX(-105%);transition:transform .22s ease;background:rgba(8,12,18,.96);box-shadow:26px 0 80px rgba(0,0,0,.45)}.app.menuOpen .side{transform:translateX(0)}.scrim{display:block;position:fixed;z-index:20;inset:0;background:rgba(0,0,0,.48);opacity:0;pointer-events:none;transition:opacity .2s}.app.menuOpen .scrim{opacity:1;pointer-events:auto}.top{height:58px;padding:0 14px;gap:12px;justify-content:flex-start}.top .meta:last-child{margin-left:auto}.chat{padding:14px}.composer{padding:12px}.box{gap:8px}.send{width:72px}.msg{max-width:100%}}
...
function renderHistory(items){history.innerHTML='';for(const item of items){const row=document.createElement('div');row.className='hist';row.title=item.title;row.addEventListener('click',()=>loadConversation(item.id));const open=document.createElement('button');open.className='histOpen';open.textContent=(item.status==='running'?'● ':'')+item.title;open.title=item.title;const del=document.createElement('button');del.className='histDelete';del.textContent='删除';del.addEventListener('click',(e)=>{e.stopPropagation();deleteConversation(item.id,item.title)});row.appendChild(open);row.appendChild(del);history.appendChild(row)}}
...
function newChat(){currentConversationId='';chat.innerHTML='<div class="empty"><b>Ask Codex</b><span>选择目录和模型，然后发送任务。</span></div>';statusEl.textContent='New chat';input.value='';input.style.height='auto';clearPendingAttachments();closeMenu();input.focus()}
async function loadConversation(id){const res=await fetch('/api/conversations/'+encodeURIComponent(id));if(!res.ok)return;const data=await res.json();currentConversationId=data.conversation.id;chat.innerHTML='';(data.conversation.messages||[]).forEach((msg,index)=>addMsg(msg.role==='log'?'log':msg.role,msg.content,{messageIndex:index}));statusEl.textContent='Loaded '+new Date(data.conversation.updatedAt||data.conversation.createdAt).toLocaleString();closeMenu()}
