...
在 Codex Web 集成中，完整皮肤由本文件生成的纯背景与 `concept-themes.json` 中对应的真实 UI 色板共同组成。选择或重新生成 `skin-01`～`skin-08` 时必须保留概念 ID，Web 才能同步应用面板、强调色、文字、描边、焦点位置和透明度；禁止用带假 UI 的效果图代替这一主题层。
